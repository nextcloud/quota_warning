OC.L10N.register(
    "quota_warning",
    {
    "Saving…" : "A gravar...",
    "Saved!" : "Gravado!",
    "Quota warning" : "Avisa de quota",
    "Nearing your storage quota" : "Está próximo da sua quota de armazenamento",
    "Or click the following button for options to change your data plan." : "Ou carregue no botão que se segue para opções para alterar o seu plano de dados.",
    "Or click the following link for options to change your data plan." : "Ou carregue na ligação que se segue para opções para alterar o seu plano de dados.",
    "Data plan options" : "Opções de plano de dados",
    "You are using more than %d%% of your storage quota" : "Está a usar mais de %d %% da sua quota de armazenamento",
    "First notification" : "Primeira notificação",
    "Send an email" : "Enviar um e-mail",
    "Second notification" : "Segunda notificação",
    "Final notification" : "Última notificação",
    "Link to quota management" : "Ligação para gestão de quota",
    "Resend notifications after … days" : "Reenviar notificações após ... dias",
    "Set to 0 if the user should only receive one notification." : "Definir como 0 se o utilizador apenas deve receber uma notificação",
    "You are using more than %d%% of your storage quota. Try to free up some space by deleting old files you don´t need anymore." : "Está a usar mais de %d %% da sua quota de armazenamento. Experimente libertar algum espaço apagando ficheiros antigos que já não precise."
},
"nplurals=2; plural=(n != 1);");
